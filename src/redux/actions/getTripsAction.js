...
const processData = (dispatch, data) => {
    const filteredData = data.map(entry => {
        const {id, route: {destination, start_point}, setoff_time, vehicle: {number_plate}, tp_fare, vehicle_id, route_id} = entry;
        const [date, time] = setoff_time.split('T');
        return {id, destination, start_point, number_plate, tp_fare: `UGX ${tp_fare}`, date, time, setoff_time, vehicle_id, route_id};
    });
    return dispatch({
